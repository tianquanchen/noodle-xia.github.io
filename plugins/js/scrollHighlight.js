	...
	$.fn.scrollHighlight = function(options) {
		return this.each(function() {
			...
			var opts = $.extend({}, defualts, options),
				...
				$con = $(opts.container),
				$w = $(window);
			if (obj.length <= 0)="" return;="" var="" resizetimer;="" set="" resizetimer="" to="" empty="" so="" it="" resets="" on="" page="" load="" item="obj.find(childItem)," i="0," len="item.length," wrap="[]," index="[]," anchor="[];" for="" (;="" <="" len;="" i++)="" {="" anchor.push(item.eq(i).attr(attribute));="" 获取需要高亮的所有节点="" }="" alen="anchor.length;" (var="" j="0;" alen;="" j++)="" that="$(anchor[j]);//container中的内容" if="" (that.length="" &&="" that.is(":visible"))="" 筛选出容器内存在的节点并且是显示的="" wrap.push(anchor[j]);="" 将节点放置在数组wrap中="" index.push(j);="" 筛选出他们的位置="" function="" myoffset(attr){="" 获取元素上边(左边)偏移值="" return="" (mode="=" 'vertical')="" ?="" $(attr).offset().top="" :="" $(attr).offset().left;="" wlen="wrap.length;" pos="[];" wlen;="" pos.push(myoffset(wrap[i]));="" 存储每个节点的偏移值="" 滚动时的函数="" onscroll(e)="" xy="(mode" =="vertical" )="" $w.scrolltop()="" +="" buffer="" $w.scrollleft()="" buffer;="" myouter(attr){="" 获取元素外部高度(宽度)="" $(attr).outerheight()="" $(attr).outerwidth();="" mypos(i)="" 当是垂直(水平)滚动时，滚动高度(宽度)大于它的上边(左边)的偏移值，高亮。="" (xy="">= pos[i]) {
